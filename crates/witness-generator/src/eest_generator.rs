...
impl FixtureGenerator for EESTFixtureGenerator {
    ...
    async fn generate(&self, witness_type: WitnessType) -> Result<Vec<Box<dyn Fixture>>> {
        ...
        let bws = tests
            .par_iter()
            .map(|(name, case)| gen_fixture(name, case, witness_type))
...
fn gen_fixture(
    ...
    let (block, witnesses) = BlockchainTestCase::run_single_case(name, case)
    ...
    let res: Box<dyn Fixture> = match witness_type {
        WitnessType::FullValidation => Box::new(StatelessValidationFixture {
            name: name.to_owned(),
        ...
        WitnessType::ExecutionOnly => Box::new(StatelessValidationFixture {
            name: name.to_owned(),
